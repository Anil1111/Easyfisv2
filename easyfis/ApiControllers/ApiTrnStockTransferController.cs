...
namespace easyfis.ModifiedApiControllers
{
    public class ApiTrnStockTransferController : ApiController
    {
        ...
        // ==============================
        [Authorize, HttpGet, Route("api/stockTransfer/dropdown/list/status")]
        public List<Entities.MstStatus> DropdownListStockTransferStatus()
        {
            var statuses = from d in db.MstStatus.OrderBy(d => d.Status)
                           where d.IsLocked == true
                           && d.Category.Equals("ST")
                            ...
                            if (toBranches.Any())
                            {
                                ...
                                if (articles.Any())
                                {
                                    ...
                                    if (users.Any())
                                    {
                                        ...
                                        String newObject = at.GetObjectString(newStockTransfer);
