...
namespace easyfis.ModifiedApiControllers
{
    public class ApiTrnStockInController : ApiController
    {
        ...
        // ==============================
        [Authorize, HttpGet, Route("api/stockIn/dropdown/list/status")]
        public List<Entities.MstStatus> DropdownListStockInStatus()
        {
            var statuses = from d in db.MstStatus.OrderBy(d => d.Status)
                           where d.IsLocked == true
                           && d.Category.Equals("IN")
                                ...
                                if (listArticleIds.Any())
                                {
                                    ...
                                    if (articleLists.Any())
                                    {
                                        ...
                                        if (users.Any())
                                        {
                                            ...
                                            String newObject = at.GetObjectString(newStockIn);
                        ...
                        if (userForms.FirstOrDefault().CanLock)
                        {
                            ...
                            if (stockIn.Any())
                            {
                                if (!stockIn.FirstOrDefault().IsLocked)
                                {
                                    ...
                                    lockStockIn.Status = objStockIn.Status;
