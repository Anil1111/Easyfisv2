                            ...
                            if (articleGroups.Any())
                            {
                                ...
                                if (units.Any())
                                {
                                    ...
                                    if (taxTypes.Any())
                                    {
                                        ...
                                        if (terms.Any())
                                        {
                                            var otherArticleByManualCode = from d in db.MstArticles
                                                                           where d.ArticleTypeId == 6
                                                                           && d.ManualArticleCode.Equals(objOtherArticle.ManualArticleCode)
                                                                           select d;

                                            if (!otherArticleByManualCode.Any())
                                            {
                                                Data.MstArticle newOtherArticle = new Data.MstArticle
                                                {
                                                    ArticleCode = defaultOtherArticleCode,
                                                    ManualArticleCode = objOtherArticle.ManualArticleCode,
                        ...
                        if (userForms.FirstOrDefault().CanEdit)
                        {
                            ...
                            if (otherArticle.Any())
                            {
                                ...
                                if (!otherArticleByManualCode.Any())
                                {
                                    ...
                                    lockOtherArticle.ManualArticleCode = objOtherArticle.ManualArticleCode;
