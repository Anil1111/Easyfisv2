...
namespace easyfis.POSIntegrationApiControllers
{
    public class ApiPOSIntegrationMstSupplierController : ApiController
    {
        ...
        public List<POSIntegrationEntities.POSIntegrationMstSupplier> GetSupplierPOSIntegration(String updateDateTime)
        {
            var suppliers = from d in db.MstArticles
                            where d.ArticleTypeId == 3
                            && d.IsLocked == true
                            && d.UpdatedDateTime.Date == Convert.ToDateTime(updateDateTime)
                            select new POSIntegrationEntities.POSIntegrationMstSupplier
                            {
                                Article = d.Article,
                                Address = d.Address,
