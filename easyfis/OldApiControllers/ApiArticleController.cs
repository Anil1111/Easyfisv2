...
namespace easyfis.Controllers
{
    public class ApiArticleController : ApiController
    {
        ...
        // list article by Package Items
        [Authorize]
        [HttpGet]
        [Route("api/article/list/byPackage")]
        public List<Models.MstArticle> listArticleByPackageItems()
        {
            var articles = from d in db.MstArticles.OrderBy(d => d.Article)
            ...
            try
            {
                ...
                if (articles.Any())
                {
                    ...
                    else if (Convert.ToInt32(articleTypeId) == 1)
                    {
                        ...
                        {
                            return Request.CreateResponse(HttpStatusCode.BadRequest, "No Unit Conversion!");
                        }
                        else
                        {
                            if (!updateArticle.MstArticlePrices.Any())
                            {
                                return Request.CreateResponse(HttpStatusCode.BadRequest, "No Price!");
                            }
                            else
                            {
                                updateArticle.ArticleCode = article.ArticleCode;
